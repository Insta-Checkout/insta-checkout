...
export function OnboardingForm() {
  ...
  const {
    ...
    defaultValues: { businessName: "", phoneNumber: "" },
  });
  ...
  const submitMinimalSignup = useCallback(
    async (firebaseUid: string, emailAddr: string) => {
      const businessName = watch("businessName")?.trim();
      const phoneRaw = watch("phoneNumber")?.trim();
      console.log("[Signup] submitMinimalSignup called", { firebaseUid, emailAddr, businessName, phoneRaw });
      if (!businessName || !phoneRaw) {
        console.log("[Signup] submitMinimalSignup aborted: missing businessName or phoneRaw");
        return false;
      }

      const phoneNumber = phoneRaw.startsWith("20")
        ? phoneRaw
        : `20${phoneRaw.replace(/^0/, "")}`;

      const apiUrl = process.env.NEXT_PUBLIC_BACKEND_URL || "http://localhost:4000";
      const postUrl = `${apiUrl}/sellers`;
      console.log("[Signup] POST /sellers to", postUrl, "| NEXT_PUBLIC_BACKEND_URL:", process.env.NEXT_PUBLIC_BACKEND_URL || "(not set, using default)");
      ...
      try {
        ...
        console.log("[Signup] POST /sellers response", { status: res.status, text: text.slice(0, 200) });
        let body: { error?: string; message?: string; details?: Array<{ field: string; message: string }> };
        try {
          body = text ? JSON.parse(text) : {};
        } catch {
          console.error("[Signup] Invalid JSON response", res.status, text);
          toast.error(t("onboard.errors.serverError"));
          return false;
        }

        if (res.status === 201) {
          console.log("[Signup] 201 Created — setting session, redirecting");
          ...
          if (user) {
            try {
              ...
              console.log("[Signup] Session cookie response", sessionRes.status);
            } catch (e) {
        ...
        if (res.status === 409) {
          console.log("[Signup] 409 Conflict", body.error);
          if (body.error === "DUPLICATE_EMAIL") {
            // Existing user signing in — seller already exists, set session and redirect
            console.log("[Signup] 409 DUPLICATE_EMAIL — setting session for existing user");
            const user = auth.currentUser;
            if (user) {
              try {
                const idToken = await user.getIdToken();
                ...
                console.log("[Signup] Session cookie (409) response", sessionRes.status);
                return true;
              } catch (e) {
                console.warn("[Signup] Failed to set session cookie:", e);
              }
        ...
        if (res.status === 400 && body.details?.length) {
          console.log("[Signup] 400 Validation error", body.details);
          toast.error(body.details.map((d) => d.message).join("، "));
          return false;
        }

        if (res.status === 500) {
          console.error("[Signup] 500 Server error", body);
          toast.error(t("onboard.errors.serverError"));
          return false;
        }

        console.error("[Signup] Unexpected response", res.status, body);
  ...
  const handleGoogleSignUp = async () => {
    console.log("[Signup] handleGoogleSignUp started");
    const businessName = watch("businessName")?.trim();
    const phoneRaw = watch("phoneNumber")?.trim();
    if (!businessName || !phoneRaw) {
      toast.error(t("onboard.validation.phoneRequired"));
      return;
    }
    if (errors.businessName || errors.phoneNumber) return;

    setIsSubmitting(true);
    try {
      console.log("[Signup] Calling signInWithGoogle...");
      const user = await signInWithGoogle();
      console.log("[Signup] Google sign-in succeeded", { uid: user.uid, email: user.email });
      if (!user.email?.trim()) {
        toast.error(t("onboard.errors.googleNoEmail"));
        return;
      }
      const ok = await submitMinimalSignup(user.uid, user.email);
      console.log("[Signup] submitMinimalSignup returned", ok);
      if (ok) {
        toast.success(t("onboard.errors.accountCreated"));
        router.replace("/dashboard");
      } else {
        console.log("[Signup] submitMinimalSignup returned false — no redirect");
      }
    } catch (err: unknown) {
  ...
  const handleEmailSignUp = async (data: z.infer<typeof emailSchema>) => {
    console.log("[Signup] handleEmailSignUp started", { email: data.email });
    const businessName = watch("businessName")?.trim();
    const phoneRaw = watch("phoneNumber")?.trim();
    if (!businessName || !phoneRaw) {
      console.log("[Signup] handleEmailSignUp aborted: missing businessName or phoneRaw");
      return;
    }
    ...
    try {
      ...
      console.log("[Signup] Firebase signUp succeeded", { uid: user.uid, email: user.email });
      const ok = await submitMinimalSignup(user.uid, user.email || data.email);
      console.log("[Signup] submitMinimalSignup returned", ok);
      if (ok) {
        toast.success(t("onboard.errors.accountCreated"));
        router.replace("/dashboard");
      ...
      if (code === "auth/email-already-in-use") {
        console.log("[Signup] Email already in use, retrying as sign-in...");
        toast.info(t("onboard.errors.emailInUseSignIn"));
        try {
          const user = await signInWithEmail(data.email, data.password);
          console.log("[Signup] Sign-in succeeded, calling submitMinimalSignup...");
          const ok = await submitMinimalSignup(user.uid, user.email || data.email);
          console.log("[Signup] submitMinimalSignup (after email-in-use) returned", ok);
          if (ok) {
    ...
    <div className="space-y-6">
      ...
      <div className="rounded-xl border border-border bg-card p-6 shadow-sm">
        <h3 className="text-lg font-bold text-foreground mb-1">
          {isSignInOnly ? t("landing.loginPage.title") : t("onboard.step2.title")}
        </h3>
        <p className="text-sm text-muted-foreground mb-4">
          {isSignInOnly ? t("landing.loginPage.subtitle") : t("onboard.step2.subtitle")}
        </p>
        ...
        {isEmailForm && (
          <motion.form
            ...
            onSubmit={(e) => {
              e.preventDefault();
              handleEmailSubmit(authMode === "signup" ? handleEmailSignUp : handleEmailSignIn)(e);
            }}
            className="space-y-4"
          >
            <div className="space-y-2">
